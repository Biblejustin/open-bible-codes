...
from scripts import check_cities_source_page_line_crop_band_map_doc as check


class CitiesSourcePageLineCropBandMapDocTests(unittest.TestCase):
    def test_current_doc_passes(self) -> None:
        assert check.validate_cities_source_page_line_crop_band_map_doc(check.DEFAULT_DOC) == []
    ...
    def test_flags_source_script_text_in_tracked_doc(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            failures = check.validate_cities_source_page_line_crop_band_map_doc(
                doc,
                rows_csv=rows,
                summary_csv=summary,
...
def copy_current_outputs(root: Path) -> tuple[Path, Path, Path, Path]:
    doc = root / "band_map.md"
    rows = root / "band_map.csv"
