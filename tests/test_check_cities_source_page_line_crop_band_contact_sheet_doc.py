...
from scripts import check_cities_source_page_line_crop_band_contact_sheet_doc as check


class CitiesSourcePageLineCropBandContactSheetDocTests(unittest.TestCase):
    def test_current_doc_passes(self) -> None:
        assert (
            check.validate_cities_source_page_line_crop_band_contact_sheet_doc(
                check.DEFAULT_DOC
            )
            == []
        )
    ...
    def test_flags_source_script_text_in_tracked_doc(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            failures = check.validate_cities_source_page_line_crop_band_contact_sheet_doc(
                doc,
                rows_csv=rows,
                summary_csv=summary,
                manifest_json=manifest,
                packet_csv=check.DEFAULT_PACKET,
                band_review_csv=check.DEFAULT_BAND_REVIEW,
            )
...
def copy_current_outputs(root: Path) -> tuple[Path, Path, Path, Path]:
    doc = root / "band_contact.md"
    rows = root / "band_contact.csv"
