...
def test_main_auto_uses_current_default_duckdb(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    pytest.importorskip("duckdb")
    hits = tmp_path / "hits.csv"
    summary = tmp_path / "summary.csv"
    md = tmp_path / "summary.md"
    manifest = tmp_path / "manifest.json"
    db = tmp_path / "reports" / "db.duckdb"
    write_csv(hits, [{"best_context": "exact_center"}, {"best_context": ""}])
    write_csv(
        summary,
        [
            {
    ...
    import_csv_table(db_path=db, csv_path=summary, table_name=default_table_name(summary))
    monkeypatch.setattr(summarize, "DEFAULT_REPORT_DB", db)
    ...
    assert code == 0
    payload = json.loads(manifest.read_text(encoding="utf-8"))
    assert payload["report_db"] == str(db)
    assert payload["aggregates"]["hit_rows"] == 2
