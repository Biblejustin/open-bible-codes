...
from scripts import check_cities_public_handoff_docs as check


def test_current_cities_public_handoff_docs_pass() -> None:
    assert check.validate_public_handoff_docs() == []
...
def test_missing_required_phrase_fails(tmp_path: Path) -> None:
    missing = "no city-name normalization, ELS searches, compactness runs, or p-levels"
...
def test_main_reports_failure(tmp_path: Path, capsys) -> None:
    ...
    assert code == 1
    assert "Cities public handoff doc failure" in capsys.readouterr().err
