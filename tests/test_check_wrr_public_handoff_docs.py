...
from scripts import check_wrr_public_handoff_docs as check


def test_current_wrr_public_handoff_docs_pass() -> None:
...
def test_missing_required_phrase_fails(tmp_path: Path) -> None:
    missing = "without choosing corrections or exclusions"
...
def test_forbidden_stale_phrase_fails(tmp_path: Path) -> None:
    stale = "all four blocker areas until source/method locks change"
...
def test_main_reports_failure(tmp_path: Path, capsys) -> None:
    ...
    assert code == 1
    assert "WRR public handoff doc failure" in capsys.readouterr().err
