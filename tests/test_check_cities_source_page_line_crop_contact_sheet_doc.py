...
from scripts import check_cities_source_page_line_crop_contact_sheet_doc as check


class CitiesSourcePageLineCropContactSheetDocTests(unittest.TestCase):
    def test_current_doc_passes(self) -> None:
        assert check.validate_cities_source_page_line_crop_contact_sheet_doc(check.DEFAULT_DOC) == []
    ...
    def test_flags_source_script_text_in_tracked_doc(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            failures = check.validate_cities_source_page_line_crop_contact_sheet_doc(
                doc,
                rows_csv=rows,
                summary_csv=summary,
    ...
    def test_flags_summary_drift(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(any("table_pages" in failure for failure in failures))
