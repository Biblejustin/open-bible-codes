...
from scripts import check_cities_source_page_line_crop_priority_review_worksheet_doc as check


class CitiesSourcePageLineCropPriorityReviewWorksheetDocTests(unittest.TestCase):
    def test_current_doc_passes(self) -> None:
        assert (
            check.validate_cities_source_page_line_crop_priority_review_worksheet_doc(
                check.DEFAULT_DOC
            )
            == []
        )
    ...
    def test_flags_source_script_text_in_tracked_doc(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            failures = check.validate_cities_source_page_line_crop_priority_review_worksheet_doc(
                doc,
                rows_csv=rows,
                summary_csv=summary,
                manifest_json=manifest,
                triage_csv=check.DEFAULT_TRIAGE,
                priority_contact_csv=check.DEFAULT_PRIORITY_CONTACT,
            )
...
def copy_current_outputs(root: Path) -> tuple[Path, Path, Path, Path]:
    doc = root / "priority_review.md"
    rows = root / "priority_review.csv"
