...
from scripts import check_cities_source_page_line_crop_triage_html_doc as check


class CitiesSourcePageLineCropTriageHtmlDocTests(unittest.TestCase):
    def test_current_doc_passes(self) -> None:
        assert check.validate_cities_source_page_line_crop_triage_html_doc(check.DEFAULT_DOC) == []
    ...
    def test_flags_source_script_text_in_tracked_doc(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            failures = check.validate_cities_source_page_line_crop_triage_html_doc(
                doc,
                html_path=html,
                summary=summary,
                manifest=manifest,
                triage=check.DEFAULT_TRIAGE,
            )
