...
from scripts import check_cities_source_page_ocr_review_packet_doc as check


class CitiesSourcePageOcrReviewPacketDocTests(unittest.TestCase):
    def test_current_doc_passes(self) -> None:
        assert check.validate_cities_source_page_ocr_review_packet_doc(check.DEFAULT_DOC) == []
    ...
    def test_flags_source_script_text_in_doc(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            failures = check.validate_cities_source_page_ocr_review_packet_doc(
                doc,
                packet=packet,
                summary=summary,
                manifest=manifest,
    ...
    def test_flags_source_script_text_in_packet_csv(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            doc, packet, summary, manifest = copy_current_outputs(root)
    ...
    def test_flags_summary_drift(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(any("source_page_ocr_rows" in failure for failure in failures))
