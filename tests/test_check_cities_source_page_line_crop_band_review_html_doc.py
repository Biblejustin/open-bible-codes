...
from scripts import check_cities_source_page_line_crop_band_review_html_doc as check


class CitiesSourcePageLineCropBandReviewHtmlDocTests(unittest.TestCase):
    def test_current_doc_passes(self) -> None:
        assert (
            check.validate_cities_source_page_line_crop_band_review_html_doc(
                check.DEFAULT_DOC
            )
            == []
        )
