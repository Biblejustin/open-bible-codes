...
from scripts import check_cities_source_page_line_crop_review_html_doc as check


class CitiesSourcePageLineCropReviewHtmlDocTests(unittest.TestCase):
    def test_current_doc_passes(self) -> None:
        assert check.validate_cities_source_page_line_crop_review_html_doc(check.DEFAULT_DOC) == []
    ...
    def test_flags_source_script_text_in_tracked_doc(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            failures = check.validate_cities_source_page_line_crop_review_html_doc(
                doc,
                html_path=html,
                summary=summary,
                manifest=manifest,
                packet=check.DEFAULT_PACKET,
            )
    ...
    def test_local_html_must_exist(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_flags_summary_drift(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(any("html_rows" in failure for failure in failures))
