...
from scripts import check_wrr_source_row_crop_review_html_doc as check


class WrrSourceRowCropReviewHtmlDocTests(unittest.TestCase):
    def test_current_doc_passes(self) -> None:
        assert check.validate_source_row_crop_review_html_doc(check.DEFAULT_DOC) == []
    ...
    def test_flags_source_script_text_in_tracked_doc(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            failures = check.validate_source_row_crop_review_html_doc(
                doc,
                html_path=html,
                summary=summary,
                manifest=manifest,
                crop_packet_path=check.DEFAULT_CROP_PACKET,
            )
...
def copy_current_outputs(root: Path) -> tuple[Path, Path, Path, Path]:
    doc = root / "row_review_html.md"
    html = root / "row_review.html"
