...
def test_missing_boundary_fails(tmp_path: Path) -> None:
    ...
    failures = check.validate_source_row_crop_packet_doc(doc)
    assert any("Crop availability is not transcription verification" in failure for failure in failures)
