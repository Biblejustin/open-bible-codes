...
from scripts import build_cities_source_page_line_crop_priority_review_html as html_builder


class CitiesSourcePageLineCropPriorityReviewHtmlTests(unittest.TestCase):
    def test_write_priority_html_uses_contact_sheet_images_without_source_text(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            image = root / "priority.png"
            image.write_bytes(b"not really png")
            rows = [contact_row("1", "priority_1_dense_text", image)]
            html_out = root / "priority_review.html"
            args = argparse_like(html_out)

            summary = html_builder.write_priority_html(rows, args)

            text = html_out.read_text(encoding="utf-8")
            self.assertEqual(summary["html_rows"], 1)
            self.assertEqual(summary["html_priority_image_rows"], 1)
    ...
    def test_main_writes_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            image = root / "priority.png"
            image.write_bytes(b"not really png")
            contact_csv = root / "priority_contact.csv"
            html_out = root / "priority_review.html"
            summary = root / "summary.csv"
            markdown = root / "priority_review.md"
            manifest = root / "manifest.json"
            write_csv(contact_csv, [contact_row("1", "priority_1_dense_text", image)])
...
def contact_row(rank: str, priority: str, image: Path) -> dict[str, str]:
    return {
        "sheet_rank": rank,
