...
from scripts import check_cities_source_page_line_crop_band_review_worksheet_doc as check


class CitiesSourcePageLineCropBandReviewWorksheetDocTests(unittest.TestCase):
    def test_current_doc_passes(self) -> None:
        assert (
            check.validate_cities_source_page_line_crop_band_review_worksheet_doc(
                check.DEFAULT_DOC
            )
            == []
        )
    ...
    def test_flags_source_script_text_in_tracked_doc(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            failures = check.validate_cities_source_page_line_crop_band_review_worksheet_doc(
                doc,
                rows_csv=rows,
                summary_csv=summary,
                manifest_json=manifest,
                band_map_csv=check.DEFAULT_BAND_MAP,
            )
...
def copy_current_outputs(root: Path) -> tuple[Path, Path, Path, Path]:
    doc = root / "band_review.md"
    rows = root / "band_review.csv"
