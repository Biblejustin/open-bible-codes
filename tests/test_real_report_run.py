...
""",
    ...
    def test_preflight_fails_on_kjva_crosswire_source_audit_doc_failure(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "preflight.json"
            with patch.object(
                preflight.check_kjva_crosswire_candidate_source_audit_doc,
                "validate_kjva_crosswire_candidate_source_audit_doc",
                return_value=["missing metadata-only boundary"],
            ):
                code = preflight.main(["--allow-dirty", "--out", str(out)])
            ...
            payload = json.loads(out.read_text(encoding="utf-8"))
            self.assertEqual(
                payload["kjva_crosswire_candidate_source_audit_doc_failures"],
                ["missing metadata-only boundary"],
            )
            self.assertIn(
    ...
    def test_preflight_fails_on_kjva_hakkaac_boundary_candidate_failure(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "preflight.json"
            with patch.object(
                preflight.check_kjva_hakkaac_apocrypha_boundary_candidate_doc,
                "validate_kjva_hakkaac_apocrypha_boundary_candidate_doc",
                return_value=["missing candidate boundary"],
            ):
                code = preflight.main(["--allow-dirty", "--out", str(out)])
            ...
            payload = json.loads(out.read_text(encoding="utf-8"))
            self.assertEqual(
                payload["kjva_hakkaac_apocrypha_boundary_candidate_doc_failures"],
    ...
    def test_preflight_fails_on_kjva_next_result_gate_failure(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "preflight.json"
            with patch.object(
                preflight.check_kjva_next_result_gate_doc,
                "validate_kjva_next_result_gate_doc",
                return_value=["missing next-result gate boundary"],
            ):
                code = preflight.main(["--allow-dirty", "--out", str(out)])
            ...
            payload = json.loads(out.read_text(encoding="utf-8"))
            self.assertEqual(
                payload["kjva_next_result_gate_doc_failures"],
                ["missing next-result gate boundary"],
            )
            self.assertIn(
    ...
    def test_preflight_fails_on_kjva_current_source_sidecar_failure(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "preflight.json"
            with patch.object(
                preflight.check_kjva_current_source_lock_sidecar_doc,
                "validate_kjva_current_source_lock_sidecar_doc",
                return_value=["missing current-source sidecar boundary"],
            ):
                code = preflight.main(["--allow-dirty", "--out", str(out)])
            ...
            payload = json.loads(out.read_text(encoding="utf-8"))
            self.assertEqual(
                payload["kjva_current_source_lock_sidecar_doc_failures"],
                ["missing current-source sidecar boundary"],
            )
            self.assertIn(
                "KJVA current source sidecar failures: missing current-source sidecar boundary",
                payload["failures"],
            )
    ...
    def test_preflight_fails_on_cities_source_page_ocr_review_html_doc_failure(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "preflight.json"
            with patch.object(
                preflight.check_cities_source_page_ocr_review_html_doc,
                "validate_cities_source_page_ocr_review_html_doc",
                return_value=[
                    "docs/CITIES_SOURCE_PAGE_OCR_REVIEW_HTML.md missing boundary"
                ],
            ):
                code = preflight.main(["--allow-dirty", "--out", str(out)])
            ...
            payload = json.loads(out.read_text(encoding="utf-8"))
            self.assertEqual(
                payload["cities_source_page_ocr_review_html_doc_failures"],
                ["docs/CITIES_SOURCE_PAGE_OCR_REVIEW_HTML.md missing boundary"],
            )
            self.assertIn(
    ...
    def test_preflight_fails_on_cities_source_page_line_crop_packet_doc_failure(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "preflight.json"
            with patch.object(
                preflight.check_cities_source_page_line_crop_packet_doc,
                "validate_cities_source_page_line_crop_packet_doc",
                return_value=[
            ...
            payload = json.loads(out.read_text(encoding="utf-8"))
            self.assertEqual(
                payload["cities_source_page_line_crop_packet_doc_failures"],
                ["docs/CITIES_SOURCE_PAGE_LINE_CROP_PACKET.md missing boundary"],
            )
            self.assertIn(
    ...
    def test_preflight_fails_on_cities_source_page_line_crop_contact_sheet_doc_failure(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "preflight.json"
            with patch.object(
                preflight.check_cities_source_page_line_crop_contact_sheet_doc,
                "validate_cities_source_page_line_crop_contact_sheet_doc",
                return_value=[
                    "docs/CITIES_SOURCE_PAGE_LINE_CROP_CONTACT_SHEET.md missing boundary"
                ],
            ):
                code = preflight.main(["--allow-dirty", "--out", str(out)])
            ...
            payload = json.loads(out.read_text(encoding="utf-8"))
            self.assertEqual(
                payload["cities_source_page_line_crop_contact_sheet_doc_failures"],
                ["docs/CITIES_SOURCE_PAGE_LINE_CROP_CONTACT_SHEET.md missing boundary"],
            )
            self.assertIn(
    ...
    def test_preflight_fails_on_cities_source_page_line_crop_band_review_worksheet_doc_failure(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "preflight.json"
            with patch.object(
                preflight.check_cities_source_page_line_crop_band_review_worksheet_doc,
                "validate_cities_source_page_line_crop_band_review_worksheet_doc",
                return_value=[
                    "docs/CITIES_SOURCE_PAGE_LINE_CROP_BAND_REVIEW_WORKSHEET.md missing boundary"
                ],
            ):
                code = preflight.main(["--allow-dirty", "--out", str(out)])
    ...
    def test_preflight_fails_on_cities_source_page_line_crop_band_contact_sheet_doc_failure(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "preflight.json"
            with patch.object(
                preflight.check_cities_source_page_line_crop_band_contact_sheet_doc,
                "validate_cities_source_page_line_crop_band_contact_sheet_doc",
                return_value=[
                    "docs/CITIES_SOURCE_PAGE_LINE_CROP_BAND_CONTACT_SHEET.md missing boundary"
                ],
            ):
                code = preflight.main(["--allow-dirty", "--out", str(out)])
            ...
            payload = json.loads(out.read_text(encoding="utf-8"))
            self.assertEqual(
                payload["cities_source_page_line_crop_band_contact_sheet_doc_failures"],
                [
                    "docs/CITIES_SOURCE_PAGE_LINE_CROP_BAND_CONTACT_SHEET.md missing boundary"
                ],
            )
    ...
    def test_preflight_fails_on_cities_source_page_line_crop_band_review_html_doc_failure(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "preflight.json"
            with patch.object(
                preflight.check_cities_source_page_line_crop_band_review_html_doc,
                "validate_cities_source_page_line_crop_band_review_html_doc",
                return_value=[
                    "docs/CITIES_SOURCE_PAGE_LINE_CROP_BAND_REVIEW_HTML.md missing boundary"
                ],
            ):
                code = preflight.main(["--allow-dirty", "--out", str(out)])
            ...
            payload = json.loads(out.read_text(encoding="utf-8"))
            self.assertEqual(
                payload["cities_source_page_line_crop_band_review_html_doc_failures"],
                [
                    "docs/CITIES_SOURCE_PAGE_LINE_CROP_BAND_REVIEW_HTML.md missing boundary"
                ],
            )
    ...
    def test_preflight_fails_on_cities_source_page_line_crop_triage_html_doc_failure(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "preflight.json"
            with patch.object(
                preflight.check_cities_source_page_line_crop_triage_html_doc,
                "validate_cities_source_page_line_crop_triage_html_doc",
                return_value=[
            ...
            payload = json.loads(out.read_text(encoding="utf-8"))
            self.assertEqual(
                payload["cities_source_page_line_crop_triage_html_doc_failures"],
                ["docs/CITIES_SOURCE_PAGE_LINE_CROP_TRIAGE_HTML.md missing boundary"],
            )
            self.assertIn(
    ...
    def test_preflight_fails_on_cities_source_page_line_crop_priority_review_html_doc_failure(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "preflight.json"
            with patch.object(
                preflight.check_cities_source_page_line_crop_priority_review_html_doc,
                "validate_cities_source_page_line_crop_priority_review_html_doc",
                return_value=[
                    "docs/CITIES_SOURCE_PAGE_LINE_CROP_PRIORITY_REVIEW_HTML.md missing boundary"
                ],
            ):
                code = preflight.main(["--allow-dirty", "--out", str(out)])
            ...
            payload = json.loads(out.read_text(encoding="utf-8"))
            self.assertEqual(
                payload["cities_source_page_line_crop_priority_review_html_doc_failures"],
                [
                    "docs/CITIES_SOURCE_PAGE_LINE_CROP_PRIORITY_REVIEW_HTML.md missing boundary"
                ],
            )
