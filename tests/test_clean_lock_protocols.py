...
def test_remaining_work_register_tracks_latest_validation_snapshot() -> None:
    ...
    assert "2623 tests" in normalized_text
    assert "2 skipped, and 29334 subtests" in normalized_text
    ...
    assert "KJVA apocrypha prospective-boundary profile JSON validation" in normalized_text
    assert "KJVA apocrypha prospective-boundary profile JSON validator" in normalized_text
    assert "STEP_TAHOT manifest cache JSON validation" in normalized_text
    ...
    assert "Bolls importer JSON decode validation" in normalized_text
    assert "Bolls ZIP translation JSON reader" in normalized_text
    assert "dynamic-span display-gloss stability" in normalized_text
    assert "do not pull long CSV definition notes" in normalized_text
    ...
    assert "Wayback recovery JSON decode validation" in normalized_text
    assert "Wayback recovery JSON decoders" in normalized_text
    ...
    assert "malformed API/raw JSON responses raise named errors" in normalized_text
    assert "CRD LLM API response JSON validation" in normalized_text
    assert "CRD OpenAI-compatible LLM client" in normalized_text
    assert "source-review HTML manifest JSON validation" in normalized_text
    assert "consolidated no-input blocker summary guarding" in normalized_text
    assert "source-review HTML/packet doc manifest readers" in normalized_text
