...
CITIES_SOURCE_PAGE_LINE_CROP_BAND_REVIEW_WORKSHEET_SUMMARY = Path(
...
CITIES_SOURCE_PAGE_LINE_CROP_REVIEW_HTML_SUMMARY = Path(
...
def cities_source_row_lock_section() -> list[str]:
    ...
    line_crop_band_review_summary = metric_dict(
        read_rows(CITIES_SOURCE_PAGE_LINE_CROP_BAND_REVIEW_WORKSHEET_SUMMARY)
    )
    ...
    line_crop_html_summary = metric_dict(
        read_rows(CITIES_SOURCE_PAGE_LINE_CROP_REVIEW_HTML_SUMMARY)
    )
    ...
    return [
        ...
        f"| No-input OCR packet pages | {no_input_summary.get('ocr_packet_pages', '0')} |",
        f"| No-input reviewed OCR packet pages | {no_input_summary.get('ocr_packet_pages_reviewed', '0')} |",
        f"| No-input unreviewed OCR packet pages | {no_input_summary.get('ocr_packet_pages_unreviewed', '0')} |",
        ...
        "`data/study/mappings/cities_source_row_lock_decisions.csv` contains",
        "the current source-row lock decisions. Formal preflight blocks any populated row that",
        ...
        f"{no_input_summary.get('ocr_packet_pages', '0')} OCR packet pages,",
        f"{no_input_summary.get('ocr_packet_pages_reviewed', '0')} reviewed OCR packet pages,",
        f"{no_input_summary.get('ocr_packet_pages_unreviewed', '0')} unreviewed OCR packet pages,",
...
def kjva_no_input_handoff_status_section(
    ...
    lines = [
        ...
        f"| Candidate source audits | {int_value(summary, 'candidate_source_audit_rows'):,} |",
        f"| Candidate verse-import-ready pages | {int_value(summary, 'candidate_source_verse_import_ready_pages'):,} |",
        f"| Candidate result-ready pages | {int_value(summary, 'candidate_source_result_ready_pages'):,} |",
        f"| CrossWire possible independent KJVA candidates | {int_value(summary, 'crosswire_possible_independent_kjva_candidates'):,} |",
        f"| Gutenberg split KJV plus Apocrypha metadata candidates | {int_value(summary, 'gutenberg_split_kjv_apocrypha_metadata_candidates'):,} |",
        f"| Wikisource source-candidate pages | {int_value(summary, 'wikisource_source_candidate_pages'):,} |",
        f"| Open-Bibles KJV/Apocrypha/deuterocanon paths | {int_value(summary, 'open_bibles_kjv_paths'):,}/{int_value(summary, 'open_bibles_apocrypha_paths'):,}/{int_value(summary, 'open_bibles_deuterocanon_paths'):,} |",
...
def wrr_audit_section(
    ...
    lines = [
        ...
        f"| Co-linear attachment PLS | `{source_hashes.get('torah_code_colinear_attachment_pls', '')}` |",
        f"| Co-linear attachment roots | `{source_hashes.get('torah_code_colinear_attachment_roots', '')}` |",
        f"| Co-linear attachment all_1698 | `{source_hashes.get('torah_code_colinear_attachment_all_1698', '')}` |",
        f"| Co-linear attachment res_113 | `{source_hashes.get('torah_code_colinear_attachment_res_113', '')}` |",
        f"| Co-linear attachment consul_138 | `{source_hashes.get('torah_code_colinear_attachment_consul_138', '')}` |",
        f"| Co-linear attachment intersec_108 | `{source_hashes.get('torah_code_colinear_attachment_intersec_108', '')}` |",
        f"| Co-linear attachment comb_143 | `{source_hashes.get('torah_code_colinear_attachment_comb_143', '')}` |",
        f"| Co-linear attachment att_heb | `{source_hashes.get('torah_code_colinear_attachment_att_heb', '')}` |",
    ...
    if source_transcription_evidence_row_summary_rows:
        total_terms = sum(
            int_values(source_transcription_evidence_row_summary_rows, "action_terms")
        )
        total_pairs = sum(
            int_values(source_transcription_evidence_row_summary_rows, "residual_pairs")
        )
        total_frontier = sum(
            int_values(source_transcription_evidence_row_summary_rows, "frontier_pairs")
        )
        first_rows = source_transcription_evidence_row_summary_rows[:5]
        lines.extend(
            [
                "",
                "Source-transcription evidence packet status:",
                "",
                ...
                f"- Row clusters: {len(source_transcription_evidence_row_summary_rows):,}.",
                "",
                "| Row | Concept | Terms | Pairs | Frontier | Read |",
                "| --- | --- | ---: | ---: | ---: | --- |",
            ]
        )
    ...
    lines.extend(
        [
            ...
            "while current manual decision records keep the working source unchanged",
