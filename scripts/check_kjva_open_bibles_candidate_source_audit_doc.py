#!/usr/bin/env python3
"""Validate seven1m/open-bibles KJVA candidate source audit boundaries."""
...
REQUIRED_PHRASES = (
    ...
    "KJV-only OSIS metadata candidate",
    "does not show apocrypha or deuterocanon coverage",
...
def main(argv: list[str] | None = None) -> int:
    ...
    failures = validate_kjva_open_bibles_candidate_source_audit_doc(
        args.doc,
        rows=args.rows,
    ...
    if failures:
        for failure in failures:
            print(f"KJVA Open-Bibles source audit doc failure: {failure}", file=sys.stderr)
        return 1
    print(f"KJVA Open-Bibles source audit doc ok: {args.doc}")
    return 0
...
def validate_kjva_open_bibles_candidate_source_audit_doc(
...
def validate_rows_csv(path: Path) -> list[str]:
    ...
    expected = {
        ...
        "source_audit_status": "kjv_only_not_kjva_source_candidate",
...
def validate_no_overclaim(doc: Path, text: str) -> list[str]:
    ...
    for line_number, line in enumerate(text.splitlines(), start=1):
        if FORBIDDEN_OVERCLAIM_RE.search(line):
