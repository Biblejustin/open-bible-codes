#!/usr/bin/env python3
"""Download Door43 English control sources from the tracked manifest."""
...
DEFAULT_MANIFEST = Path("configs/door43_english_controls.csv")
DEFAULT_RAW_DIR = Path("data/raw/door43")
USER_AGENT = "Open Bible Codes Door43 source importer"
...
def write_source_manifest(
    ...
    *,
    ...
    manifest = {
        ...
        "normalization": "USFM markers, notes, crossrefs, and word attributes removed; verse text preserved.",
    }
    path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")


def write_run_manifest(args: argparse.Namespace, *, rows: list[str]) -> None:
    path = Path("reports/door43_english_controls/download.manifest.json")
    path.parent.mkdir(parents=True, exist_ok=True)
    manifest = {
        "tool": "download_door43_english_controls",
