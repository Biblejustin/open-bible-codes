#!/usr/bin/env python3
"""Download eBible English control sources from the tracked manifest."""
...
DEFAULT_MANIFEST = Path("configs/ebible_english_controls.csv")
DEFAULT_RAW_DIR = Path("data/raw/ebible")
...
def download_row(row: dict[str, str], args: argparse.Namespace) -> str:
    ...
    zip_path = args.raw_dir / f"{source_id}_usfm.zip"
...
def write_run_manifest(args: argparse.Namespace, *, rows: list[str]) -> None:
    path = Path("reports/ebible_english_controls/download.manifest.json")
    path.parent.mkdir(parents=True, exist_ok=True)
    manifest = {
        "tool": "download_ebible_english_controls",
