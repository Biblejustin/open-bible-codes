# Compound Extension Prospective Preregistration

Status: registered clean-lock candidate-discovery screen.

This document freezes the Hebrew Gospel/genealogy clean-lock cohort before the
result-producing protocol. The fixed value here is that the term list, source
set, skip range, candidate rule, and representative-control rule were declared
before this specific controlled report was run.
...
| Preregistration commit | recorded by `reports/study_locks/compound_extension_prospective.manifest.json` |
