# Greek Expanded Surface Available Control Evaluation

Status: exploratory matched-control evaluation; no claim.

This report compares the tightened surface-triage targets against their
frozen same-length, real-word, surface-frequency-matched controls. The
primary statistic is all-source exact-center surface pattern count.

Matched controls per target range from 30 to 32. The best possible add-one empirical p-values in this run are 0.030303, 0.032258.

| Term | Concept | Observed all-source | Controls >= observed | p_ge | q | Read |
| --- | --- | ---: | ---: | ---: | ---: | --- |
| `ισαακ` (Isaak; English: Isaac) | Isaac | 1 | 0 | 0.030303 | 0.032258 | exploratory matched controls below p<=0.05; still post-screen |
| `ανομια` (anomia; English: Lawlessness) | Lawlessness | 1 | 0 | 0.032258 | 0.032258 | exploratory matched controls below p<=0.05; still post-screen |
| `τερασ` (teras; English: Wonder) | Wonder | 1 | 0 | 0.030303 | 0.032258 | exploratory matched controls below p<=0.05; still post-screen |

## Read

Rows where no matched control reaches the observed all-source pattern
count are useful triage evidence, but this remains post-screen unless
the control size and selection rule were frozen in advance.
