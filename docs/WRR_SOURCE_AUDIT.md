...
- WRR perturbation triples, perturbed offsets, first-ten row widths,
  ...
  real-corpus domain-labeling diagnostics, domain-weighted `Q(w,w')` summation
  for domain-labeled ELS rows, and the corrected-distance rank step for
  already-computed perturbation proximities, including the strict WRR 1994
  `v/m` rank and the tie-aware methodology-page diagnostic rank.
