...
- highlight rows: 5
- controlled or partial catalog rows: 13
...
| `greek_lexicon_common_pronoun_extension` | `partially_reproducible` | Original clean lock produced two all-source autou pronoun extension families that passed Stage A and Stage B numeric controls; strict Greek stopword rerun dropped seven stopword rows and left zero all-source extension keys | `docs/GREEK_LEXICON_EXTENSION_PROSPECTIVE_REPORT.md` |
| `all_codes_yom_yhwh_compound_extension` | `controlled_review_candidate` | Selected key passed locked 5000/5000 confirmatory controls in all five MT-family sources with conservative all-control q = 0.004799 | `docs/ALL_CODES_COMPOUND_EXTENSION_CONFIRMATORY_CONTROLS.md` |
| `crd_exact_center_word_broad_screen` | `partially_reproducible` | Self-surface and concept-surface CRD produce identical 1153 Bible exact center-word term-hit rows; collapsed counts are 154 term IDs / 87 distinct normalized surface forms / 496 distinct normalized surface hit paths; center-word-only summary has 141 terms above secular max and 114 Bible-positive secular-zero terms; concept expansion changes zero exact center-word exceedance decisions | `docs/CRD_CENTER_WORD_SELF_VS_CONCEPT_FINDINGS.md` |
...
| `local_business_place_terms` | `partially_reproducible` | Local appendix found Hebrew Cowboy only; Cowboy is not unusual under representative controls; Hebrew Catering/Cowboy Catering/Simsberry/Simscorner and all Greek local forms are absent | `docs/LOCAL_TERMS_APPENDIX_REPORT.md` |
...
| `critical_omission_breakage` | `partially_reproducible` | Five follow-up analyses now run through shared break-stat helpers; base omitted-verse breakage has 558 broken TR hits after excluding adjacent-merge and renumbered-subscription rows; 1000-shuffle null gives p_ge 0.9910 and null median 657; reverse cross-tradition length-stratified and disputed-passage override reports exist | `docs/CRITICAL_OMISSION_BREAKS.md` |
