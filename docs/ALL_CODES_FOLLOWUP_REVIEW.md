...
| 37 | greek_screening | `span_context_review` | `σαλα` (Sala; English: Shelah) | Shelah | 2 | Acts 7:42 | `Ἰσραήλ` (israel; English: Israel) | BYZ_NT,SBLGNT,TCG_NT,TR_NT | `α` (a) (after_match; SBLGNT) | all_source; start-to-end span carries exact/related surface context; adjacent same-skip extension `α` (a) in SBLGNT (after_match) |
...
| 40 | greek_screening | `hidden_path_review` | `σαλα` (Sala; English: Shelah) | Shelah | 2 | 1Cor 10:18 | `Ἰσραὴλ` (israel; English: Israel) | BYZ_NT,SBLGNT,TCG_NT,TR_NT | `α` (a) (after_match; SBLGNT) | all_source; no surface echo required; paths audited in 4 corpora; adjacent same-skip extension `α` (a) in SBLGNT (after_match) |
...
| 43 | hebrew_screening | `strongest_manual_review` | `שממה` (shmmh; English: Desolation) | Desolation | 2 | Mic 1:7 | `שְׁמָמָ֑ה` (shmmh; English: Desolation) | EBIBLE_WLC,MAM,MT_WLC,UHB,UXLC | `עבה` (bh) (before_match; UXLC) | all_source; hidden term centered on same normalized surface word; adjacent same-skip extension `עבה` (bh) in UXLC (before_match) |
...
| 66 | hebrew_screening | `span_context_review` | `תתתתתכז` (tttttkz; English: Gregorian 2027 additive) | Gregorian 2027 additive | 86 | Deut 11:16 | `יִפְתֶּ֖ה` (ypth) | EBIBLE_WLC,MAM,MT_WLC,UHB,UXLC | `שה` (shh; English: Lamb) (after_match; UXLC) | all_source; start-to-end span carries exact/related surface context; adjacent same-skip extension `שה` (shh; English: Lamb) in UXLC (after_match) |
...
| 74 | hebrew_theology | `related_center_word_review` | `ברית` (bryt; English: Covenant) | Covenant | 8 | Deut 34:9 | `חָכְמָ֔ה` (chkmh; English: Wisdom) | EBIBLE_WLC,MAM,MT_WLC,UHB,UXLC | `בריתו` (brytw) (term_plus_after; UXLC) | all_source; center surface word is related by concept/category flag; compound same-skip extension `בריתו` (brytw) in UXLC |
