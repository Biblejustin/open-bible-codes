...
- Current `make release-ready` passed, including `python3 -m pytest -q`: 2590
...
- Tightened the WRR manual decision-register document checker so a malformed
  manifest reports an explicit invalid-JSON failure instead of escaping from
  the WRR documentation gate.
- Tightened the WRR source row crop-packet document checker so malformed
...
- Tightened the WRR source row coverage-packet document checker so malformed
  manifest JSON and non-object JSON roots report explicit manifest failures
  instead of escaping from the WRR documentation gate.
...
- Tightened the WRR claim-readiness, method-status, and lock-options document
  ...
  gate.
- Tightened all remaining WRR doc JSON readers so malformed manifest JSON and
  non-object JSON roots report explicit manifest failures instead of escaping
  from the WRR documentation gate.
- Tightened the Cities doc JSON readers so malformed manifest JSON and
  non-object JSON roots report explicit manifest failures instead of escaping
  from the Cities documentation gate.
- Tightened the KJVA doc manifest validators so malformed manifest JSON and
  non-object JSON roots report explicit manifest failures instead of escaping
  from the KJVA documentation gate.
...
- Added a shared script JSON object reader and routed remaining direct
...
- Tightened report-index row-count cache parsing so malformed cache entries are
  skipped and invalid JSON report files are indexed as error rows instead of
  stopping the index build.
...
- Tightened prospective lane-status profile JSON validation so non-object
  ...
  gate.
- Tightened the study-lock manifest-drift audit doc checker so non-object
  manifest JSON roots report explicit failures instead of crashing the drift
  audit gate.
- Tightened the KJVA apocrypha prospective-boundary profile JSON validator so
  non-object profile files report explicit failures instead of crashing the
  boundary gate.
