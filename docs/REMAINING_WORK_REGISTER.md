...
records, and the first two approved Cities source-page locks:

- Current `make fast-validate` passed, including `python3 -m pytest -q`: 1967
  tests, 2 skipped, and 29325 subtests.
- `cities_source_row_lock_001` and `cities_source_row_lock_002` are now locked
  as `source_row_lock_ready` for later source-row extraction review only;
